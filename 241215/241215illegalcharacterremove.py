...
def sanitize_excel_text(text):
    """Remove or replace illegal characters that cannot be used in Excel."""
    if not isinstance(text, str):
        return text  # Only process strings
    # Remove control characters and any other invalid characters
...
def get_thread_replies(client, channel_id, thread_ts):
    """Fetch replies for a thread given the thread_ts (timestamp)."""
    try:
        response = client.conversations_replies(channel=channel_id, ts=thread_ts)
        return response["messages"]
    except SlackApiError as e:
        print(f"Error fetching thread replies for thread {thread_ts} in channel {channel_id}: {e.response['error']}")
        return []
...
for channel in selected_channels:
    ...
    for msg in messages:
        ...
        text = sanitize_excel_text(msg.get("text", ""))  # Sanitize text content
        ...
        message_texts.append({
            ...
            "text": text,
            "date": date_time,
            "is_reply": False,  # Main message
            "parent_message": None
        })

        # Fetch replies if thread_ts exists
        if "thread_ts" in msg and msg["thread_ts"] == msg["ts"]:
            thread_replies = get_thread_replies(client, channel_id, msg["thread_ts"])
            for reply in thread_replies[1:]:  # Skip the main message
                reply_user = reply.get("user", "Unknown")
                reply_text = sanitize_excel_text(reply.get("text", ""))  # Sanitize reply text
                reply_timestamp = float(reply["ts"])
                reply_date_time = datetime.fromtimestamp(reply_timestamp).strftime('%Y-%m-%d %H:%M:%S')

                message_texts.append({
                    "user": reply_user,
                    "text": reply_text,
                    "date": reply_date_time,
                    "is_reply": True,  # Reply
                    "parent_message": text
                })
